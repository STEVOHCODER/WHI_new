...
async function getSubmissions() {
  ...
  if (safe.error) {
    console.error("[admin contacts] mongo error:", safe.error.message);
    return [];
  }
  const db = safe.db!;
  return (await db
    ...
    >
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div className="flex-1 min-w-0">
          ...
          <p className="mt-1 text-sm text-[var(--color-text-muted)]">
            From: <span className="font-semibold text-[var(--color-text)]">{submission.name}</span> ·{" "}
...
function MarkReadForm({ submissionId, read }: { submissionId: string; read: boolean }) {
  return (
    <form action={`/api/admin/contacts/${submissionId}/read`} method="POST" className="inline">
